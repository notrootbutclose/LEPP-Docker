...
Internet → Nginx (reverse proxy) → PHP-FPM (x2) → MariaDB
                ↓                        ↓           ↓
             Redis (sessions)     Prometheus ← Exporters
                                        ↓
                                    Grafana
                                        ↓
                                  Loki ← Promtail (logs)
...
2. Nginx → PHP-FPM (x2)
    ...
    The PHP application:
        ...
        Reads/writes data to MariaDB.
        Stores sessions in Redis (instead of on local disk—faster and more scalable).

3. PHP-FPM → MariaDB

    MariaDB is a relational database management system (a fork of MySQL).
    The PHP application connects to it to store and retrieve structured data: users, posts, orders, etc.
    It’s important to use connection pooling, prepared statements, and proper access privileges for security.
...
5. Monitoring: Exporters → Prometheus → Grafana
    ...
    Exporters (e.g., node_exporter, php-fpm_exporter, mysql/mariadb_exporter) are lightweight agents that:
        Collect metrics (CPU, RAM, PHP-FPM request rates, slow MariaDB queries, etc.).
    ...
    Grafana:
        ...
        Visualizes metrics in intuitive dashboards (CPU usage graphs, PHP response times, etc.).

6. Logging: Promtail → Loki → Grafana

    Promtail:
        An agent running on each server/container that reads log files (e.g., /var/log/php-fpm.log, /var/log/nginx/access.log).
        Ships them to Loki.
    Loki:
...
│   └── prometheus.yml
│
├── mysqld-exporter/            # MySQL exporter config
│   └── .my.cnf
│
